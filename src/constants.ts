export const RACQUET_BRANDS = [
  "Wilson",
  "Babolat",
  "Yonex",
  "Head",
  "Prince",
  "Tecnifibre",
  "Dunlop",
  "Volkl",
  "Solinco",
  "ProKennex"
];

export const RACQUET_MODELS: Record<string, string[]> = {
  "Wilson": ["Pro Staff", "Blade", "Ultra", "Clash", "Burn", "Shift"],
  "Babolat": ["Pure Drive", "Pure Aero", "Pure Strike", "Pure Aero Rafa"],
  "Yonex": ["EZONE", "VCORE", "VCORE PRO", "Percept", "Astrel"],
  "Head": ["Speed", "Radical", "Extreme", "Prestige", "Gravity", "Boom", "Instinct"],
  "Prince": ["Phantom", "Tour", "Warrior", "Ripstick"],
  "Tecnifibre": ["TFight", "TF40", "Tempo", "T-Rebound"],
  "Dunlop": ["FX", "SX", "CX"],
  "Volkl": ["V-Cell", "V-Feel"],
  "Solinco": ["Whiteout", "Blackout"],
  "ProKennex": ["Ki", "Black Ace"]
};

export const STRINGS = [
  { brand: "Luxilon", models: ["Alu Power", "4G", "Element", "Smart", "Adrenaline"] },
  { brand: "Babolat", models: ["RPM Blast", "VS Gut", "RPM Rough", "RPM Power", "RPM Hurricane", "Origin"] },
  { brand: "Wilson", models: ["NXT", "Sensation", "Revolve", "Natural Gut", "Champions Choice"] },
  { brand: "Yonex", models: ["Poly Tour Pro", "Poly Tour Strike", "Poly Tour Rev", "Poly Tour Air", "Dynawire"] },
  { brand: "Head", models: ["Lynx", "Hawk", "Velocity MLT", "Reflex MLT", "Sonic Pro"] },
  { brand: "Solinco", models: ["Hyper-G", "Tour Bite", "Confidential", "Barb Wire", "Vanquish"] },
  { brand: "Tecnifibre", models: ["NRG2", "X-One Biphase", "Razor Code", "Black Code", "Triax"] }
];
